/* NetId(s): sk2545 Time spent: hh hours, mm minutes.
 * Name(s): Sena Katako
 */
...
public class Heap<V> {
	...
	public void add(V v, double p) throws IllegalArgumentException {
		...
		Info member = new Info(v, p);
		valPos.put(member.value, size);
		c[size] = member;
		size++;
		bubbleUp(size -1);
	...
	private void bubbleUp(int k) {
		...
		while(k > 0 && c[k].priority < c[parent].priority) {
			swap(k,parent);
			k = parent;
			parent= (k-1)/2;
		}
	...
	public V poll() {
		...
		V element = peek();
		valPos.remove(element);

		Info member = c[size - 1];
		c[0] = member;
		c[size - 1] = null;
		bubbleDown(0);
		size--;
...
//		int child2 = 2*k + 2;
		...
		Info kval = c[k];
		Integer mapk = valPos.get(kval);
		
		while(2*k + 1 < size){
			int child = smallerChildOf(k);
			Info childval = c[child];
			Integer mapchild = valPos.get(childval);
	...
	protected int smallerChildOf(int n) {
		...
		double leftChildPriority= valPos.get(c[leftchild].priority);
		double rightChildPriority = valPos.get(c[rightchild].priority);
